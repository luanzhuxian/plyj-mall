...
export default {
    ...
    computed: {
        ...
        currentSkuModel () {
            return this.data.skuModels.find(item => item.skuCode1 === this.data.cartSkuCode && item.skuCode2 === this.data.cartSkuCode2) || {}
        },
        stock () {
        ...
        currentImage () {
            const { cartSkuCode } = this.data
            try {
                const currentSkuAttr = this.data.productAttributes[0].productAttributeValues.find(item => item.id === cartSkuCode)
