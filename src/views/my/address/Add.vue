...
export default {
    ...
    data () {
        return {
            ...
            form: {
                ...
                // stirng 收货人名称
                realName: '',
                // stirng 地址标识 家庭地址HOME，工作地址WORK, 其他地址：OTHER
                addressLable: 'HOME',
                defaultAddress: 'NO',
                // 街道、镇、乡
                town: ''
    ...
    methods: {
        ...
        async confirm () {
            if (this.$refs.form.validate()) {
                ...
                try {
                    ...
                    if (this.addressId) {
                        const { result } = await updateAddress(this.form, this.addressId)
                        currentAddress = result
                    } else {
                        const { result } = await createAddress(this.form)
                        currentAddress = result
                    }
        ...
        selectCity (val) {
            ...
            this.form.addressPrefix = val[0].name +
        val[1].name +
        (val[2] ? val[2].name : '') +
        (val[3] ? val[3].name : '')
        },
        ...
        async removeAddr () {
            try {
                ...
                const { result } = await deleteAddress(this.addressId)
                if (result) {
                    this.$success('已删除')
