...
export default {
    ...
    methods: {
        ...
        selectedChange (id) {
            if (this.activeMenuId === 1) {
                const index = this.findCurrentCouponIndex(id, this.usedCouponList)
                const currentCoupon = this.usedCouponList.splice(index, 1)[0]
                currentCoupon.checked = !currentCoupon.checked
                this.usedCouponList.splice(index, 0, currentCoupon)
            } else {
                const index = this.findCurrentCouponIndex(id, this.overduedCouponList)
                const currentCoupon = this.overduedCouponList.splice(index, 1)[0]
                currentCoupon.checked = !currentCoupon.checked
        ...
        async deleteCoupon () {
            try {
                const needDeleteLists = this.activeMenuId === 1 ? this.usedCouponList : this.overduedCouponList
                const deleteList = needDeleteLists.filter(item => item.checked).map(item => item.id)
    ...
    watch: {
        ...
        isManagementState (val) {
            if (val) {
                const listName = this.activeMenuId === 1 ? 'usedCouponList' : 'overduedCouponList'
