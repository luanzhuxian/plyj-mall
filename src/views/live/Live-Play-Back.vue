    ...
    methods: {
        async isNeedPay () {
            try {
                ...
                const { result: { needPay, needPaidAmount, paidAmount, activityName, isHaveToken, isInputToken } } = await hasPied(this.activityId, !!this.isValidateEndTime)

                // 是否口令校验
                await this.getPermission(isHaveToken, isInputToken)
                // 单位分转为元
                this.payCount = needPaidAmount / 100
