<template>
  <div :class="$style.home + ' home'">
    ...
    <top-text :title="mallName" :tip="moment().format('MM月DD日 dddd')"></top-text>
    <!-- banner -->
          ...
          <template v-for="(item, index) of banner">
            <swiperSlide :key="index">
              <div :class="$style.swiperSlide">
                ...
                />
                <!-- 其它 -->
                <img v-else v-lazy="item.fileId" alt="">
                <div :class="$style.bannerInfo">
                  <div v-if="item.productName || item.title">
                    <div :class="$style.name" v-text="item.productName || item.title"></div>
                    <Grade size="mini" :grade="item.grade || 5"></Grade>
                  </div>
                  <div v-if="item.productDesc" :class="$style.desc" v-text="item.productDesc"></div>
                </div>
              </div>
            </swiperSlide>
          </template>
    ...
    <div :class="$style.module" v-if="modules[0]" style="margin-top: 16vw">
      <module-title size="middle" title="甄选好礼" />
      <feature-lesson-item
