<template>
    <div v-if="likeProduct.length > 0" :class="$style.like">
        <div v-if="isMy" :class="$style.titleMy">
            <img v-imgError src="https://penglai-weimall.oss-cn-hangzhou.aliyuncs.com/static/mall/2.0.0/my/youLike.png">
        </div>
...
export default {
    ...
    methods: {
        ...
        minPrice (item) {
            const priceList = item.productSkuModels.map(item => item.price).sort((a, b) => a - b)
