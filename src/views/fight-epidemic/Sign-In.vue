...
export default {
    ...
    async activated () {
        try {
            const flag = await this.getSignInInfo()
            if (flag) {
    ...
    methods: {
        ...
        async getSignInInfo () {
            try {
                const { result } = await getSignInInfo(this.id)
        ...
        async getactivityInfo () {
            try {
                const { result } = await getactivityInfo(this.id)
        ...
        async getGiftInfo () {
            try {
                const { result } = await getGiftInfo(this.id, this.$store.getters.userId)
        ...
        async getBarrage () {
            try {
                const { result: { records } } = await getBarrage(this.id)
        ...
        async signIn (content) {
            try {
                const { status, result: { status: ActivityStatus } } = await signIn({
                    activityId: this.id,
                    content
                })
                if (status === 200) {
